...
type AIActivity = { specialties: readonly { key: string; count: number }[]; completed: number; pending: number; review: number };

export function HealixAIWidget({ activity, t, formatNumber }: { activity: AIActivity; t: TranslationFunction; formatNumber: (value: number) => string }) {
  ...
  return (
    <section className="surface-section ai-widget overflow-hidden" aria-labelledby="ai-heading">
      ...
      <div className="px-5 py-4">
        <p className="mb-4 flex items-baseline gap-2"><span className="text-2xl font-semibold">{formatNumber(total)}</span><span className="clinical-caption">{t("dashboard.clinical.ai.today")}</span></p>
        ...
        <div className="mt-4 border-t border-[var(--line-soft)] pt-3">
          <p className="text-xs font-medium text-[var(--warning-ink)]">{t("dashboard.clinical.ai.review", { count: formatNumber(activity.review) })}</p>
          <p className="clinical-caption mt-1">{t("dashboard.clinical.ai.completed", { count: formatNumber(activity.completed) })} · {t("dashboard.clinical.ai.pending", { count: formatNumber(activity.pending) })}</p>
        </div>
