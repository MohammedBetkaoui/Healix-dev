...
import { establishmentNavSections } from "../layout/navigation";
...
export function EstablishmentPatientsPage() {
  ...
  return (
    <DashboardShell
      ...
      breadcrumbLabel={t("patients.page.breadcrumb")}
      user={{
        accountType: "ESTABLISHMENT",
            ...
            <div className="overflow-x-auto">
              <table className="min-w-full divide-y divide-slate-200 text-sm">
                <thead className="bg-slate-50 text-xs uppercase tracking-[0.12em] text-slate-500">
                  <tr>
                    <th className="px-5 py-4 text-start">{t("patients.table.columns.fullName")}</th>
                    <th className="px-5 py-4 text-start">{t("patients.table.columns.id")}</th>
                    <th className="px-5 py-4 text-start">{t("patients.table.columns.gender")}</th>
                    <th className="px-5 py-4 text-start">{t("patients.table.columns.birthDate")}</th>
                    <th className="px-5 py-4 text-start">{t("patients.table.columns.assignedDoctor")}</th>
                    <th className="px-5 py-4 text-start">{t("patients.table.columns.status")}</th>
                    <th className="px-5 py-4 text-start">{t("patients.table.columns.lastVisit")}</th>
                    <th className="px-5 py-4 text-start">{t("patients.table.columns.phone")}</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-100">
                  {filteredPatients.map((patient) => (
                    <tr key={patient.id} className="align-top">
                      ...
                      <td className="px-5 py-4 text-slate-600">{t(`patients.genders.${patient.gender}`)}</td>
                      <td className="px-5 py-4 text-slate-600">{formatDate(patient.birthDate, locale)}</td>
